...
function normalize_rows(mixed $rows, int $count): array {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        $defaults[$i] = [
            ...
            'ua_fines' => clean_num($row['ua_fines'] ?? ''),
...
<div class="page">
    ...
    <div class="ledger-row">
        ...
        <table class="ledger" aria-label="UA column">
            ...
            <tbody>
            <?php for ($i = 0; $i < $rowCount; $i++): ?>
                <tr><td class="right w-small"><input type="text" data-row="<?= $i ?>" data-field="ua_fines" inputmode="decimal"></td></tr>
            <?php endfor; ?>
            </tbody>
        </table>
...
function collectRows() {
    ...
    for (let i = 0; i < rowCount; i++) {
        rows.push({
            ...
            ua_fines: field(i, 'ua_fines').value,
