...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'create_house') {
        ...
        $templateDatabase = trim((string)($_POST['template_database'] ?? 'secretary')) ?: 'secretary';
        ...
        if ($houseName === '' || $managerName === '' || $managerEmail === '' || $managerPassword === '') {
            ...
            try {
                ...
                $houseCode = $houseCode !== '' ? oxford_generate_unique_house_code($masterPdo, $houseCode) : oxford_generate_unique_house_code($masterPdo, $houseName);
                ...
                $databaseName = $databaseNameInput !== '' ? $databaseNameInput : oxford_generate_unique_database_name($masterPdo, $oxfordServerPdo, $houseName);
                ...
                $insertHouse = $masterPdo->prepare('INSERT INTO oxford_master_houses (house_name, house_code, database_name, city, state, is_active) VALUES (?, ?, ?, ?, ?, 1)');
                $insertHouse->execute([$houseName, $houseCode, $databaseName, $city, $state]);
                $houseId = (int)$masterPdo->lastInsertId();

                $insertUser = $masterPdo->prepare('INSERT INTO oxford_master_users (full_name, email, password_hash, role, status) VALUES (?, ?, ?, ?, ?)');
                $insertUser->execute([$managerName, $managerEmail, password_hash($managerPassword, PASSWORD_DEFAULT), 'house_manager', 'active']);
                $userId = (int)$masterPdo->lastInsertId();

                $linkUser = $masterPdo->prepare('INSERT INTO oxford_master_house_user_access (house_id, user_id, is_primary) VALUES (?, ?, 1)');
...
$houses = $masterPdo->query('SELECT h.*, (SELECT COUNT(*) FROM oxford_master_house_user_access hua WHERE hua.house_id = h.id) AS assigned_users FROM oxford_master_houses h ORDER BY h.house_name ASC')->fetchAll();
$totalHouses = count($houses);
$activeUsers = (int)$masterPdo->query("SELECT COUNT(*) FROM oxford_master_users WHERE status='active'")->fetchColumn();
$totalAuditRows = (int)$masterPdo->query('SELECT COUNT(*) FROM oxford_master_audit_log')->fetchColumn();
$totalActivityRows = (int)$masterPdo->query('SELECT COUNT(*) FROM oxford_master_activity')->fetchColumn();
...
foreach ($houses as $house) {
    ...
    $totalRows = 0;
    try {
        $tables = $oxfordServerPdo->query("SELECT TABLE_NAME, TABLE_ROWS FROM information_schema.TABLES WHERE TABLE_SCHEMA = " . $oxfordServerPdo->quote($db))->fetchAll();
        $tableRowsMap = [];
        foreach ($tables as $t) {
...
$recentActivity = $masterPdo->query('SELECT a.*, h.house_name, u.full_name FROM oxford_master_activity a LEFT JOIN oxford_master_houses h ON h.id = a.house_id LEFT JOIN oxford_master_users u ON u.id = a.user_id ORDER BY a.created_at DESC LIMIT 20')->fetchAll();
$recentAudit = $masterPdo->query('SELECT l.*, h.house_name, u.full_name FROM oxford_master_audit_log l LEFT JOIN oxford_master_houses h ON h.id = l.house_id LEFT JOIN oxford_master_users u ON u.id = l.user_id ORDER BY l.created_at DESC LIMIT 25')->fetchAll();
?>
<style>
.dashboard-wrap{max-width:1500px;margin:0 auto;} .dashboard-title{margin:0 0 8px;color:#17365d;} .dashboard-sub{color:#647992;margin-bottom:20px;} .stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:18px;} .stat-card,.panel{background:#fff;border-radius:18px;box-shadow:0 10px 26px rgba(15,44,80,.08);} .stat-card{padding:18px;} .stat-label{font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#6f8298;margin-bottom:8px;} .stat-value{font-size:30px;font-weight:700;color:#17365d;} .panel{padding:18px;margin-bottom:18px;} .table-wrap{overflow:auto;} table{width:100%;border-collapse:collapse;} th,td{padding:11px 10px;border-bottom:1px solid #e7eef6;text-align:left;white-space:nowrap;vertical-align:top;} th{font-size:12px;text-transform:uppercase;color:#6c7f96;letter-spacing:.06em;} .badge{display:inline-block;padding:5px 9px;border-radius:999px;background:#e9f1fb;color:#17365d;font-size:12px;font-weight:700;} .open-btn,.submit-btn{display:inline-block;padding:10px 14px;border-radius:10px;background:#17365d;color:#fff;text-decoration:none;font-weight:700;border:0;cursor:pointer;} .open-btn:hover,.submit-btn:hover{background:#244d7d;} .muted{color:#71859a;} .split{display:grid;grid-template-columns:1.25fr .95fr;gap:18px;} .triple{display:grid;grid-template-columns:1.1fr .9fr;gap:18px;margin-bottom:18px;} .alert{padding:14px 16px;border-radius:14px;margin-bottom:16px;font-weight:700;} .alert.success{background:#eef8ef;border:1px solid #cfe8d2;color:#27653a;} .alert.error{background:#fff2f2;border:1px solid #efcccc;color:#a63333;} .form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;} .form-grid .full{grid-column:1/-1;} label{display:block;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6c7f96;margin-bottom:6px;} input,select{width:100%;padding:12px 13px;border:1px solid #d9e4f1;border-radius:12px;font-size:14px;} .cred-box{background:#f5f8fc;border:1px solid #dce7f3;border-radius:14px;padding:14px;margin-top:16px;line-height:1.7;} @media(max-width:1100px){.split,.triple,.form-grid{grid-template-columns:1fr;}}
</style>
<div class="dashboard-wrap">
            ...
            <form method="post">
                ...
                <div class="form-grid">
                    ...
                    <div>
                        ...
                        <input disabled type="text" name="template_database" value="oxford_tables" placeholder="Database to copy tables from (usually secretary or oxford_tables)">
                    </div>
                    <div>
                        <label>Address</label>
                ...
                <div style="margin-top:14px;"><button class="submit-btn" type="submit">Create House, Database, Tables, and Login</button></div>
            </form>
    ...
    <div class="split">
        <div class="panel">
            ...
            <div class="table-wrap">
                <table>
                    <thead><tr><th>Module</th><th>Total Records Across Houses</th></tr></thead>
                    <tbody>
                    <?php foreach ($knownTables as $tableName => $label): ?>
                        <tr><td><strong><?= oxford_h($label) ?></strong></td><td><?= (int)$moduleTotals[$tableName] ?></td></tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
            </div>
        ...
        <div class="panel">
            ...
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr><th>When a house is created</th><td>The system registers the house, builds a dedicated database, clones the template table structure,<br> and creates the initial core member login.</td></tr>
            ...
            <table>
                ...
                <?php foreach ($houseBreakdown as $house): ?>
                    <tr>
                        ...
                        <!-- <td><span class="badge"><?= oxford_h($house['database_name']) ?></span></td> -->
    ...
    <div class="split">
        <div class="panel">
            ...
            <div class="table-wrap">
                <table>
                    <thead><tr><th>Date</th><th>House</th><th>User</th><th>Page</th><th>Event</th><th>Details</th></tr></thead>
                    <tbody>
        ...
        <div class="panel">
            ...
            <div class="table-wrap">
                <table>
                    <thead><tr><th>Date</th><th>House</th><th>User</th><th>Action</th><th>Page</th><th>Details</th></tr></thead>
                    <tbody>
